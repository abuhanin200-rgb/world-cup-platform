...
export default function AdminChallengeStudioPanel() {
  ...
  generatedByAI: true,
      ...
      alert(`تم توليد نشرة بالذكاء الاصطناعي ✅\nعدد البطاقات: ${data.cards.length}`);
  ...
  return (
    <section className="rounded-3xl border border-white/10 bg-white/10 p-4 shadow-2xl md:p-5">
      <div className="mb-5 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="text-xl font-black md:text-2xl">🎙️ استوديو التحدي</h2>
          <p className="mt-1 text-sm text-slate-300">
            توليد نشرات مبنية على بيانات البطولة الحقيقية، ثم تعديلها ومعاينتها ونشرها.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-2 md:flex-row">
          ...
          >
            {generatingAi ? "جاري كتابة النشرة..." : "🤖 توليد بالذكاء الاصطناعي"}
          </button>
      ...
      <div className="mb-5 rounded-2xl border border-cyan-400/20 bg-cyan-400/10 p-3 text-xs font-bold leading-6 text-cyan-100 md:text-sm">
        الآن تقدر تولّد النشرة، تعدّل النصوص والبطاقات قبل النشر، ثم تنشرها للأعضاء.
      </div>
            ...
            {editCards.map((card, index) => (
              ...
              >
                ...
                <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
                  <label>
                    <span className="mb-1 block text-xs font-bold">النوع</span>
                    <select
                      value={card.type}
                      onChange={(event) =>
                        updateEditCard(
                  ...
                  <label>
                    <span className="mb-1 block text-xs font-bold">الأيقونة</span>
                    <input
                      value={card.icon}
                      onChange={(event) =>
                  ...
                  <label>
                    <span className="mb-1 block text-xs font-bold">الأولوية</span>
                    <input
                      type="number"
                      value={card.priority}
                      onChange={(event) =>
            ...
            >
              <div className="mb-3 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate-400">{bulletin.date}</p>
                </div>
