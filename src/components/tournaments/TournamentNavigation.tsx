...
const SKIP_GULF27_INTRO_ONCE_KEY = "altahaddi:gulf27:skip-intro-once";
...
export default function TournamentNavigation({ tournament, activeSection = "home" }: Props) {
  ...
  const markInternalReturnToOverview = (itemKey: TournamentSection | "home") => {
    if (tournament.slug !== "gulf-cup-27" || itemKey !== "home" || activeSection === "home") return;

    try {
      window.sessionStorage.setItem(SKIP_GULF27_INTRO_ONCE_KEY, "1");
    } catch {
          ...
          {ITEMS.map((item) => {
            ...
            return (
              <Link
                ...
                onClick={() => markInternalReturnToOverview(item.key)}
              ...
              >
                <Icon className={`h-3.5 w-3.5 md:h-4 md:w-4 ${active ? "text-[var(--tournament-primary)]" : ""}`} aria-hidden="true" />
                {item.label}
              </Link>
            );
          })}
