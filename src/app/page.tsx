...
// 🕋 مصفوفة قاعدة البيانات المعتمدة والآمنة مية بالمية لحل خطأ السطر 467 نهائياً
const FIXTURES_365_DATABASE = [
  ...
  { id: "wc_05", day: "الأحد 14 يونيو", group: "كأس Candidate - دور المجموعات", team1: "هايتي", team1Emoji: "🇭🇹", team2: "اسكتلندا", team2Emoji: "🏴󠁧󠁢󠁳󠁣󠁴󠁿", time: "04:00 ص", kickoff: "2026-06-14T04:00:00" },
...
const WORLD_CUP_2026_TEAMS = [
  ...
  { code: "GB", name: "إنجلترا", emoji: "🏴󠁧󠁢󠁥الن󠁧󠁿" }, { code: "PT", name: "البرتغال", emoji: "🇵🇹" },
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    // دمج مباريات الأدمن السحابية لايف ومنع خطأ التصفية الحركي
    ...
    const unsubUsers = onSnapshot(query(collection(db, "users")), (snap) => {
      const rawUsers = snap.docs.map(doc => ({ id: doc.id, name: doc.data().fullName, teamEmoji: doc.data().teamEmoji || "🏆", total: doc.data().total || 0, correct: doc.data().correct || 0, wrong: doc.data().wrong || 0, points: doc.data().points || 0, rankDirection: doc.data().rankDirection || "➖", rankChange: doc.data().rankChange || 0 }));
  ...
  const handleGuestLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const userData = { fullName: targetName, favoriteTeam: user.favoriteTeam || "السعودية 🇸🇦", teamEmoji: chosenEmoji, password: user.password, phone: user.phone || "", points: 0, total: 0, correct: 0, wrong: 0, currentRank: 99, previousRank: 99, rankDirection: "💡 جديد", rankChange: 0, createdAt: new Date().toISOString() };
  ...
  // 🛠️ تثبيت دالة الخروج handleLogout وتصفير الخطأ الأول بشاشتك نهائياً
  const handleLogout = () => {
  ...
  const handleSavePredictionForMatch = async (matchId: string, team1: string, team1Emoji: string, team2: string, team2Emoji: string, kickoff: string) => {
    ...
    if (firebaseUserPredictions[matchId]) { alert("لقد قمت بحفظ توقعك مسبقاً سحابياً!"); return; }
  ...
  // 🛠️ تثبيت وتصليح دالة الشات handleSendMessage وتصفير الخطأ الثاني بشاشتك نهائياً
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-gradient-to-b from-slate-950 via-purple-950 to-slate-950 text-slate-100 font-sans antialiased text-right flex flex-col justify-between select-none">
      ...
      {showConfetti && (
        <div className="fixed inset-0 z-50 pointer-events-none bg-purple-500/10 backdrop-blur-sm flex items-center justify-center">
          <div className="text-center bg-slate-950/90 border border-yellow-500 rounded-3xl p-8 shadow-2xl animate-bounce">
            <h2 className="text-2xl font-black text-yellow-400">🎉 احتفالية فوز كبرى! 🎉</h2>
            <p className="text-sm font-bold text-white mt-2">حصلت على 3 نقاط كاملة بالتوقع بالملي!</p>
          </div>
        </div>
      )}
                      ...
                      <div className="flex items-center justify-center flex-1 w-full py-1">
                        {savedPred ? (
                          <div className="text-center py-1.5 animate-fade-in">
                            <span className="text-slate-400 font-bold text-xs">{match.team1} {match.team1Emoji} </span>
                            <span className="font-mono bg-purple-950 px-3 py-1 rounded-xl text-green-400 font-black text-sm">{savedPred.score1} - {savedPred.score2}</span>
                            <span className="text-slate-400 font-bold text-xs"> {match.team2Emoji} {match.team2}</span>
