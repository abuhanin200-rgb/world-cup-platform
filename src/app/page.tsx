...
// 🕋 جدول بطولة كأس العالم 2026 الحقيقي مية بالمية من الفيفا وقوقل المعتمد بتوقيت مكة المكرمة
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const unsubUsers = onSnapshot(query(collection(db, "users")), (snap) => {
      const rawUsers = snap.docs.map(doc => ({ id: doc.id, name: doc.data().fullName, teamEmoji: doc.data().teamEmoji || "🏆", total: doc.data().total || 0, correct: doc.data().correct || 0, wrong: doc.data().wrong || 0, points: doc.data().points || 0, favoriteTeam: doc.data().favoriteTeam }));
      
      // 📊 تعديل حاسم: ترتيب الصدارة بناءً على (أعلى نقاط ➔ ثم الأكثر توقعات صحيحة ➔ ثم الأكثر مشاركة الكلية)، ومن لم يشارك يكون تحت تلقائياً
      ...
      // إسناد المراكز التسلسلية بعد الترتيب بدقة فائقة
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-gradient-to-b from-slate-950 via-purple-950 to-slate-950 text-slate-100 font-sans antialiased text-right flex flex-col justify-between select-none">
      ...
      <div>
        {/* Header والقوائم المنسدلة لجميع التبويبات مستعادة بالكامل */}
        ...
        {/* شريط آخر التوقعات بالأرقام والأعلام حية لايف */}
        ...
        {activeTab === "main_screen" && (
          <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
                ...
                {next48HoursMatches.map((match: any) => (
                  <div key={match.id} className="bg-slate-900/80 backdrop-blur-xl rounded-xl p-3 shadow-lg border border-purple-500/10 flex flex-col md:flex-row items-center justify-between gap-3 text-center">
                    ...
                    <div className="flex flex-row md:flex-col items-center justify-between md:justify-center md:w-48 w-full border-t md:border-t-0 md:border-r border-white/5 pt-2 md:pt-0 gap-2 flex-shrink-0">
                      ...
                      {!userPredictionsKeys[match.id] && (
                        <button onClick={() => handleSavePredictionForMatch(match.id, match.team1, match.match1Emoji || match.team1Emoji, match.team2, match.match2Emoji || match.team2Emoji, match.kickoff)} className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-black px-3 py-1.5 rounded-lg text-[10px] shadow-md transition-all interactive-btn">إرسال التوقع 🚀</button>
                      )}
                    </div>
                  </div>
                ))}
            ...
            {/* لوحة الصدارة العامة المكونة من أول 20 اسماً مع عمود "المركز" المستقل */}
                    ...
                    <table className="w-full text-xs text-center border-collapse">
                      <thead>
                        <tr className="bg-purple-950/40 text-purple-300 font-black border-b border-purple-900/30">
                          {/* 🎖️ تعديل: وضع "المركز" كقائمة تسلسلية منسقة مستقلة وعرض العلم الحماسي بجانب الاسم */}
                          <th className="py-2.5 px-2 text-right w-16">المركز</th>
                          <th className="py-2.5 px-2 text-right">الاسم والعلم المرشح</th>
                      ...
                      <tbody className="divide-y divide-purple-950/20 font-bold text-slate-300">
                        {slicedLeaderboard.map((u: any, i: number) => (
                          <tr key={i} className="hover:bg-purple-950/20 transition-all">
                            <td className="py-3 px-2 text-right font-black text-amber-500 text-sm bg-purple-950/10 rounded-r-lg">#{u.rank}</td>
                            <td className="py-3 px-2 text-right font-black text-white flex items-center gap-1.5"><span>{u.name}</span> <span className="text-sm">{u.teamEmoji}</span></td>
      ...
      <footer className="bg-slate-950 text-slate-500 py-6 mt-12 border-t border-purple-900/20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex flex-col sm:flex-row justify-between items-center gap-4">
          <div className="text-xs text-center sm:text-right">
            ...
            <p className="text-slate-600">© جميع الحقوق محفوظة • فكرة وتطوير عبدالسلام العنزي</p>
          </div>
        </div>
      </footer>
    </div>
