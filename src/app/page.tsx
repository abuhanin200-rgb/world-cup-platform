...
// 📅 قاعدة البيانات الرسمية والمصححة 100% لجولة الافتتاح بتوقيت مكة المكرمة (معدل خطأ 0)
const FIXTURES_365_DATABASE = [
  { day: "اليوم — الخميس 11 يونيو", group: "كأس العالم - المجموعة أ", team1: "المكسيك", team1Emoji: "🇲🇽", team2: "جنوب أفريقيا", team2Emoji: "🇿🇦", time: "10:00 م", note: "المباراة الإفتتاحية الرسمية لكأس العالم 2026 🏟️" },
  { day: "غداً — الجمعة 12 يونيو", group: "كأس العالم - المجموعة ب", team1: "كندا", team1Emoji: "🇨🇦", team2: "البوسنة والهرسك", team2Emoji: "🇧🇦", time: "11:00 م", note: "أولى مباريات الأراضي الكندية 🇨🇦" },
  { day: "السبت، 13 يونيو", group: "كأس العالم - المجموعة د", team1: "الولايات المتحدة", team1Emoji: "🇺🇸", team2: "باراغواي", team2Emoji: "🇵🇾", time: "04:00 ص", note: "افتتاحية مباريات أمريكا الفجر 🇺🇸" }
];
...
export default function HomePage() {
  ...
  const handleGuestLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!snap.empty) { alert("❌ عذراً! هذا الاسم مسجل مسبقاً في لوحة الصدارة."); return; }
  ...
  const handleUpdateUserProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("✅ تم تعديل وتحديث بيانات ملفك الشخصي بنجاح!");
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-gradient-to-b from-slate-950 via-purple-950 to-slate-950 text-slate-100 font-sans antialiased text-right flex flex-col justify-between">
      ...
      {/* ⚡ التعديل: الحركة انسيابية مقبولة وبطيئة (45s) وتتحرك من اليسار إلى اليمين بنجاح مية بالمية */}
      <style>{`
      ...
      <div>
          ...
          <div className="w-full relative overflow-hidden flex items-center">
            <div className="forced-marquee-right gap-10 items-center">
              {livePredictions.length === 0 ? (
                ...
                livePredictions.map((p: any, idx: number) => (
        ...
        {activeTab === "main_screen" && (
          <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
            ...
            {/* 📺 مركز البث الرقمي المباشر لقنوات فيفا لقسم الـ Live Match */}
            <section className="bg-gradient-to-b from-slate-900 to-slate-950/80 rounded-2xl p-6 shadow-2xl border border-purple-500/20 relative overflow-hidden">
              <div className="flex items-center justify-between mb-4 border-b border-purple-900/30 pb-3">
                <h3 className="font-black text-xs md:text-sm text-amber-400 flex items-center gap-2">📺 البث الرقمي ومركز المباراة المباشر</h3>
                <div className="flex items-center gap-2 bg-red-950 text-red-400 px-3 py-1 rounded-full text-[10px] font-black border border-red-500/20 animate-pulse">
                  <span className="w-1.5 h-1.5 bg-red-500 rounded-full"></span> تحديث فوري لايف
                </div>
              </div>
              <div className="flex items-center justify-center gap-6 md:gap-12 py-4 bg-slate-950/40 rounded-xl border border-white/5">
                <div className="flex flex-col items-center gap-2 w-28 md:w-40 text-center">
                  <span className="text-4xl md:text-5xl drop-shadow">{currentMatch.team1Emoji}</span>
                  <span className="font-black text-xs md:text-sm truncate w-full">{currentMatch.team1}</span>
                </div>
                <div className="flex flex-col items-center gap-1">
                  <div className="bg-purple-950/80 border border-purple-500/30 px-6 py-2 rounded-2xl text-green-400 font-mono font-black text-xl md:text-3xl tracking-widest shadow-2xl shadow-purple-500/10">{currentMatch.score}</div>
                  <span className="text-[10px] text-slate-400 font-bold mt-1 bg-white/5 px-2.5 py-0.5 rounded-md">{currentMatch.status}</span>
                </div>
                <div className="flex flex-col items-center gap-2 w-28 md:w-40 text-center">
                  <span className="text-4xl md:text-5xl drop-shadow">{currentMatch.team2Emoji}</span>
                  <span className="font-black text-xs md:text-sm truncate w-full">{currentMatch.team2}</span>
                </div>
              </div>
            </section>
            ...
            <section className="bg-slate-900/60 backdrop-blur-xl rounded-2xl p-5 md:p-6 shadow-2xl border border-purple-500/20 relative overflow-hidden">
              ...
              {hasPredicted ? (
                ...
                <>
                  <div className="text-center md:text-right mb-4 mt-4 md:mt-0"><h3 className="text-xs md:text-base font-black text-purple-300 flex items-center gap-1.5 justify-center md:justify-start">🔥 شارك توقعك الآن</h3></div>
        ...
        {/* 📅 التبويب 2: جدول مباريات كأس العالم 2026 المحدث 100% والموثق بتوقيت مكة (تصفير الخطأ) */}
        {activeTab === "match_fixtures" && (
          <main className="max-w-4xl mx-auto px-4 py-6 space-y-6 animate-fade-in">
            <div className="bg-slate-900/60 p-4 rounded-2xl border border-purple-500/20 shadow-2xl">
              <h3 className="font-black text-sm text-amber-400 mb-4 border-b border-purple-900/20 pb-2 flex items-center gap-2">📅 جدول مواعيد جولة افتتاح كأس العالم 2026 (توقيت مكة الرسمي)</h3>
                      ...
                      {FIXTURES_365_DATABASE.map((fixture: any, idx: number) => (
                        <div key={idx} className="space-y-2 text-right px-1">
                          ...
                          <div className="bg-slate-950 border border-purple-900/20 rounded-xl p-4 flex flex-col md:flex-row items-center justify-between text-center gap-3">
                            ...
                            <div className="text-[10px] font-bold text-amber-400 w-24 text-left"><span>{fixture.note || "مجدولة"}</span></div>
                          </div>
                        </div>
                      ))}
                    </div>
                  ) : (
                    apiFixtures.map((fixture: any, idx: number) => (
                      <div key={idx} className="space-y-2 text-right px-1">
                        <div className="text-[11px] font-black text-purple-400 bg-purple-950/30 px-3 py-1 rounded-lg inline-block">{fixture.day}</div>
                        <div className="bg-slate-950 border border-purple-900/20 rounded-xl p-4 flex flex-col md:flex-row items-center justify-between text-center gap-3">
                          <div className="text-[10px] text-slate-500 font-bold md:w-32 text-right">{fixture.group}</div>
                          ...
                          <div className="text-[10px] font-bold w-24 text-left"><span className={fixture.note ? "text-red-500 animate-pulse" : "text-amber-400"}>{fixture.note || fixture.status}</span></div>
                        </div>
                      </div>
                    ))
                  )}
      ...
      {/* 👤 الفوتر المعتمد والثابت لتوثيق حقوق واسم عبدالسلام العنزي في كافة التبويبات */}
          ...
          <div className="bg-slate-900 border border-purple-500/30 w-full max-w-md rounded-2xl p-5 md:p-6 shadow-2xl relative max-h-[90vh] overflow-y-auto text-slate-100">
            ...
            {authMode === "menu" && (
              <div className="space-y-6 py-4 text-center">
                ...
                <div className="space-y-3 pt-2">
                  <button onClick={() => setAuthMode("guest")} type="button" className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-black py-3 rounded-xl text-xs md:text-sm transition-all shadow-lg flex items-center justify-center gap-2"><span>🟢</span> التسجيل السريع</button>
            ...
            {authMode === "manual_login" && (
              <form onSubmit={handleManualLogin} className="space-y-4 py-2 text-right">
                ...
                <div><label className="block text-[11px] font-bold text-purple-300 mb-1">الاسم المطابق للحساب</label><input type="text" required placeholder="ادخل اسمك المسجل بدقة" value={manualName} onChange={(e) => setManualName(e.target.value)} className="w-full bg-slate-950 border border-purple-500/20 rounded-xl px-4 py-2.5 text-slate-100" /></div>
                <div><label className="block text-[11px] font-bold text-purple-300 mb-1">الرقم السري الخاص بالحساب</label><input type="password" required placeholder="ادخل رقمك السري" value={manualPassword} onChange={(e) => setManualPassword(e.target.value)} className="w-full bg-slate-950 border border-purple-500/20 rounded-xl px-4 py-2.5 text-slate-100" /></div>
      ...
      {/* 👤 مودال تحرير بيانات الأعضاء والجوال الحصري */}
      {isProfileModalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-amber-500/30 w-full max-w-md rounded-2xl p-5 md:p-6 shadow-2xl relative text-slate-100 text-right">
            ...
            <form onSubmit={handleUpdateUserProfile} className="space-y-3.5 text-xs font-bold">
              ...
              <div><label className="text-purple-300 block mb-1">ترشيحك لبطل كأس العالم 2026 🏆</label><select value={editProfileFields.favoriteTeam} onChange={(e)=>setEditProfileFields({...editProfileFields, favoriteTeam: e.target.value})} className="w-full bg-slate-950 border border-purple-500/20 p-2.5 rounded-xl text-white focus:outline-none">{WORLD_CUP_2026_TEAMS.map((t: any, i: number) => ( <option key={i} value={t.name}>{t.emoji} {t.name}</option> ))}</select></div>
