...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    // ⚡ الحل الجذري: جلب كافة الأعضاء بلا استثناء وحذف شرط الترتيب الذي كان يحجب الحسابات القديمة
        ...
        <div className="lg:col-span-2 bg-slate-900 p-4 rounded-xl border border-purple-500/10 h-[395px] flex flex-col shadow-md">
          ...
          <div className="overflow-y-auto flex-grow text-xs hidden-scrollbar overflow-x-auto">
            <table className="w-full text-center border-collapse min-w-[500px]">
              <thead><tr className="bg-slate-950/50 text-slate-400 font-bold border-b border-white/5"><th className="py-2 text-right">الاسم المسجل / الزائر</th><th>الرقم السري الحقيقي</th><th>النقاط</th><th>خيارات السيطرة المطلقة</th></tr></thead>
                ...
                {users.length === 0 ? (
                  ...
                  users.map((u, i) => (
                    <tr key={i} className="hover:bg-white/5 transition-colors">
                      ...
                      <td className="font-mono text-purple-400 font-bold bg-purple-950/20 px-2 py-0.5 rounded-lg border border-purple-500/10 inline-block mt-1.5">
                      ...
                      <td className="space-x-1 space-x-reverse"><button onClick={()=>updateUserPoints(u.id, u.points || 0)} className="bg-blue-900/40 text-blue-300 border border-blue-500/20 hover:bg-blue-900/60 px-2.5 py-1 rounded-lg font-bold">تعديل النقاط ⚙️</button><button onClick={()=>deleteItem("users", u.id)} className="bg-red-900/40 text-red-300 border border-red-500/20 hover:bg-red-900/60 px-2.5 py-1 rounded-lg font-bold">طرد وحذف 🗑️</button></td>
                    </tr>
                  ))
                )}
              </tbody>
