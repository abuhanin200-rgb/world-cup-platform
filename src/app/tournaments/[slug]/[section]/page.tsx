...
export default async function TournamentSectionPage({ params }: Props) {
  ...
  return <main dir="rtl" style={getTournamentThemeStyle(tournament)} className="min-h-screen bg-[var(--tournament-background)] text-[var(--tournament-text)]"><TournamentHero tournament={tournament}/><TournamentNavigation tournament={tournament} activeSection={section}/><TournamentSectionHeader tournament={tournament} section={section}/><div className="mx-auto max-w-7xl px-3 sm:px-4 md:px-6"><TournamentStatsSummary tournament={tournament}/></div>{tournament.engine === "legacy_wc2026" ? <WorldCup2026Section section={section}/> : <TournamentV2Section tournament={tournament} section={section}/>}</main>;
}
