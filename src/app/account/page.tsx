...
export default function AccountPage() {
  const router=useRouter(); const {user,isLoggedIn,loading,refreshUser,secureSession}=useAuth();
  ...
  const [editing,setEditing]=useState(false); const [fullName,setFullName]=useState(""); const [phone,setPhone]=useState(""); const [email,setEmail]=useState(""); const [favoriteTeam,setFavoriteTeam]=useState(""); const [message,setMessage]=useState(""); const [saving,setSaving]=useState(false);
  ...
  useEffect(()=>{if(!user||!secureSession)return;let active=true;void getMemberPrivateProfile(user.id).then(profile=>{if(active)setEmail(profile?.email||"")}).catch(error=>console.error("تعذر تحميل البريد الخاص:",error));return()=>{active=false}},[user,secureSession]);
  ...
  async function saveProfile(e:FormEvent){e.preventDefault();if(!user){setMessage("تعذر العثور على بيانات الحساب. سجّل الدخول مرة أخرى.");return;}if(!isValidEmail(email)){setMessage("أدخل بريدًا إلكترونيًا صحيحًا");return;}setSaving(true);setMessage("");try{await updateUserProfile({userId:user.id,fullName,phone,favoriteTeam,teamEmoji:user.teamEmoji});await updateMemberEmail(user.id,email);await refreshUser();setMessage("تم حفظ بياناتك بنجاح");setEditing(false)}catch(err){setMessage(err instanceof Error?err.message:"تعذر حفظ البيانات")}finally{setSaving(false)}}
